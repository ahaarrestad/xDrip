...
> **Note:** The original spec estimated ~71 files for this category. A codebase-wide search (`resources.getColor`, `getResources().getColor`, `context.getColor`, `getDrawable` variants) found only this one file. All other `getColor`/`getDrawable` calls either already use `ContextCompat`/`AppCompatResources`/`ResourcesCompat`, or are unrelated method names. No Phase 2 needed for Category 1.
...
- [x] **Step 2: Run the tests — confirm they pass on original code**
...
- [x] **Step 1: Add the ContextCompat import**
...
- [x] **Step 2: Replace all 6 `resources.getColor()` calls**
...
- [x] **Step 3: Run tests again — confirm still green**
...
- [x] **Step 3: Open PR targeting `master`** — **[NightscoutFoundation/xDrip#4478](https://github.com/NightscoutFoundation/xDrip/pull/4478)**
...
**PR target:** All PRs go against the upstream repo (`NightscoutFoundation/xDrip`), not the fork.

**Before opening a PR:** Always ask the user for confirmation first.
